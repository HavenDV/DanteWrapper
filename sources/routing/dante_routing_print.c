...
(
	...
	{
		offset += SNPRINTF(buf+offset, len-offset, "%c%u", (i ? ',' : '['), (unsigned long) sockets->read_fds.fd_array[i]);
	}
#else
...
(
	...
	const char * default_name = dr_device_get_default_name(device);

	static const char * k_name_not_available = "<not available>";

	if (! actual_name)
		actual_name = k_name_not_available;
	if (! connect_name)
		connect_name = "[localhost]";
	if (! advertised_name)
		advertised_name = k_name_not_available;
	if (! default_name)
		default_name = k_name_not_available;
	...
	DR_TEST_PRINT(
		...
		"  Advertised name: %s\n"
		"  Default name:    %s\n"
		, actual_name, connect_name, advertised_name, default_name
	);
}
...
(
	...
	unsigned int a, num_addresses = 2;
	...
	if (dr_device_is_local_connection(device))
	{
		DR_TEST_PRINT("  Address=local\n");
	}
	else if (IS_MANAGED_DOMAIN_UUID(dr_devices_get_domain_uuid(dr_device_get_devices(device))))
	{
		dante_domain_routing_id_t routing_id;
		result = dr_device_get_domain_routing_id(device, &routing_id);
		if (result == AUD_SUCCESS)
		{
			DR_TEST_PRINT("  Domain Routing Id: 0x%08x.%d\n", routing_id.endpoint_id, routing_id.component_id);
		}
		else
		{
			DR_TEST_ERROR("  Error getting domain routing id: %s\n", aud_error_get_name(result));
		}
		
	}
	else
	{
		result = dr_device_get_addresses(device, &num_addresses, addresses);
		DR_TEST_PRINT("  Address");
		if (result == AUD_SUCCESS)
		{
			for (a = 0; a < num_addresses; a++)
			{
				uint8_t * host = (uint8_t *)&(addresses[a].host);
				DR_TEST_PRINT("%c%u.%u.%u.%u:%u",
					(a ? ',' : '='),
					host[0], host[1], host[2], host[3], addresses[a].port);
			}
			DR_TEST_PRINT("\n");
		}
		else
		{
			DR_TEST_ERROR("  Error getting addresses: %s\n", aud_error_get_name(result));
		}
	}
	...
	if (state == DR_DEVICE_STATE_ACTIVE)
	{
		...
		DR_TEST_PRINT("    rx latency min: %dus\n", dr_device_get_rx_latency_min_us(device));
		...
		DR_TEST_PRINT("    unicast performance: %s\n", dr_device_can_set_unicast_performance(device) ? "Read/Write" : "Read Only");
...

		{
			...
			result = dr_device_get_aes67_mcast_prefix(device, &ipv4_prefix); 
			if(result == AUD_SUCCESS)
			{
				DR_TEST_PRINT("    aes67 prefix:   0x%x\n", ipv4_prefix);
			}
			else
			{
				DR_TEST_PRINT("    aes67 prefix:  N/A\n");
			}
		}

		if (dr_device_is_rtp_supported(device))
		{
			if (dr_device_is_rtp_enabled(device))
			{
				DR_TEST_PRINT("    RTP Status:   Enabled\n");
			}
			else
			{
				DR_TEST_PRINT("    RTP Status:   Supported\n");
			}
		}
		else
		{
			DR_TEST_PRINT("    RTP Status:   Not Supported\n");
		}
...
(
	...
	/*
	uint16_t ii;
	set_output_array_length(sizeof(tx_channel_info_t), 3, array, count);
	for (ii = 0; ii < 3; ii++)
	{
		...
		copy_to_output_array(ii, &info, sizeof(tx_channel_info_t), array);
	}
	return;
	*/
	...
	enum
	{
		ID_FIELD_WIDTH = 2,
		NAME_FIELD_WIDTH = 20,
		FORMAT_FIELD_WIDTH = 20,
		ENABLED_FIELD_WIDTH = 6,
		MUTED_FIELD_WIDTH = 6,
		DBU_FIELD_WIDTH = 6
	};
	...
	DR_TEST_PRINT("  %*s %*s %*s %*s %*s %*s\n", 
		-ID_FIELD_WIDTH,      "ID",
		-NAME_FIELD_WIDTH,    "Name",
		-FORMAT_FIELD_WIDTH,  "Format",
		-ENABLED_FIELD_WIDTH, "Enable",
	...
	for (i = 0; i < n; i++)
	{
		...
		else
		{
			...
			dante_dbu_t dbu = dr_txchannel_get_signal_reflevel(txc);
	...
	*/
	...
	for (i = 0; i < n; i++)
	{
		...
		else
		{
			...
			dante_dbu_t dbu = dr_rxchannel_get_signal_reflevel(rxc);
			char flow_buf[32];
...
(
	...
	DR_TEST_PRINT("  %*s %*s %*s %*s %*s %*s %*s %*s %*s %*s\n",
		-ID_FIELD_WIDTH,      "ID",
		-NAME_FIELD_WIDTH,    "Name",
		...
		-TRANSPORT_FIELD_WIDTH, "Transport:AES67_ID",
	...
	for (f = 0; f < nf; f++)
	{
		...
		result = dr_txflow_get_flow_class(flow, &flow_class);

		if (result != AUD_SUCCESS)
		{
			DR_TEST_PRINT("Error getting flow transport type: %s\n",
				dr_error_message(result, g_test_errbuf));
			dr_txflow_release(&flow);
			return;
		}
		...
		{
			result = dr_txflow_get_aes67_sdp_origin_addr(flow, &origin_addr);
			
			if (result != AUD_SUCCESS)
			{
				DR_TEST_PRINT("Error getting SAP message id hash: %s\n",
					dr_error_message(result, g_test_errbuf));
				dr_txflow_release(&flow);
				return;
			}
		...
		DR_TEST_PRINT("  %*d %*s %*s %*s %*u %*u %*s %*s %*s",
			-ID_FIELD_WIDTH, id,
			...
			-TRANSPORT_FIELD_WIDTH, transport_buf
		);
...
(
	...
	for (f = 0; f < nf; f++)
	{
		...
		result = dr_rxflow_get_flow_class(flow, &flow_class);
		if (result != AUD_SUCCESS)
		{
			DR_TEST_ERROR (" Error getting rx transport type : %s\n",
				dr_error_message(result, g_test_errbuf));
			dr_rxflow_release(&flow);
			return;
		}
		...
		if (flow_class == DANTE_FLOW_CLASS__AES67_MCAST_IP)
		{
			...
			result = dr_rxflow_get_aes67_sdp_origin_addr(flow, &aes67_origin_addr);

			if (result != AUD_SUCCESS)
			{
				DR_TEST_PRINT("Error getting SDP origin address %s\n",
					dr_error_message(result, g_test_errbuf));
				dr_rxflow_release(&flow);
				return;
			}

			SNPRINTF(source, DANTE_NAME_LENGTH * 2, "%d.%d.%d.%d", 
				aes67_origin_addr & 0xff, (aes67_origin_addr >> 8) & 0xff, (aes67_origin_addr >> 16) & 0xff, (aes67_origin_addr >> 24) & 0xff);
		}
		else 
		{	
			aes67_origin_addr = 0;
		...
		{
			SNPRINTF(transport_buf, sizeof(transport_buf), "%s:%02x%02x%02x%02x", "AES67"
				, aes67_origin_addr & 0xff
				, (aes67_origin_addr >> 8) & 0xff
				, (aes67_origin_addr >> 16) & 0xff
		...
		DR_TEST_PRINT("  %*d %*s %*s %*s %*s %*s %*s %*d %*s",
			-ID_FIELD_WIDTH, id,
			-NAME_FIELD_WIDTH, (name ? name : "-"),
...
(
	...
	if (flow_id)
	{
		...
		for (i = 0; i < ni; i++)
		{
			...
			dante_rxflow_error_type_t error_type;
			for (error_type = 0; error_type < DR_RXFLOW_ERROR_FIELD_COUNT; error_type++)
			{
				uint32_t value;
				dante_rxflow_error_timestamp_t timestamp;
				if (available_rxflow_error_fields & (1 << error_type))
				{
					result = dr_rxflow_get_error_field_uint32(flow, i, error_type, &value, &timestamp);
					if (result != AUD_SUCCESS)
					{
						DR_TEST_ERROR("Error getting flow interface field %d: %s\n",
							error_type, dr_error_message(result, g_test_errbuf));
						return;
					}
	...
	else
	{
		...
		for (error_type = 0; error_type < DR_RXFLOW_ERROR_FIELD_COUNT; error_type++)
		{
			if (available_rxflow_error_fields & (1 << error_type))
			{
				result = dr_device_get_rxflow_error_fields(device, error_type, &all_fields, &timestamp);
				if (result != AUD_SUCCESS)
				{
					DR_TEST_ERROR("Error getting flow interface field %d: %s\n",
						error_type, dr_error_message(result, g_test_errbuf));
				...
				DR_TEST_PRINT("%s: %u.%u:", 
					dante_rxflow_error_type_to_string(error_type), timestamp.seconds, timestamp.subseconds);
				for (i = 0; i < num_intfs; i++)
				{
					uint16_t f;
					for (f = 0; f < max_rxflows; f++)
