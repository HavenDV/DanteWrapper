...
// Suppress "unreferenced parameter" warnings on windows
#ifdef WIN32
#pragma warning(push)
...
static conmon_aud_print_msg_t k_print_table [] =
{
	...
	{
		CONMON_AUDINATE_MESSAGE_TYPE_UPGRADE_V3_CONTROL,
		& conmon_aud_print_msg_upgrade_v3_control,
		"UPGRADE_V3_CONTROL",
		NULL
	},
...

	{
		...
		& conmon_aud_print_msg_sys_reset,
		"SYS_RESET",
		NULL
	},
	...
	{
		CONMON_AUDINATE_MESSAGE_TYPE_TX_SIGNAL_REFLEVEL_CHANGE,
		& conmon_aud_print_msg_id_value32,
		"TX_SIGNAL_REFLEVEL_CHANGE"
	},

	{
		CONMON_AUDINATE_MESSAGE_TYPE_RX_SIGNAL_REFLEVEL_CHANGE,
		& conmon_aud_print_msg_id_value32,
		"RX_SIGNAL_REFLEVEL_CHANGE"
	},
	...
	{
		CONMON_AUDINATE_MESSAGE_TYPE_AES67_STATUS,
		& conmon_aud_print_msg_aes67_status,
		"AES67_STATUS",
		NULL
	},
	{
		CONMON_AUDINATE_MESSAGE_TYPE_LOCK_STATUS,
		& conmon_aud_print_msg_lock_status,
		"LOCK_STATUS",
		NULL
	}, 
	{
		CONMON_AUDINATE_MESSAGE_TYPE_TX_CHANNEL_LABEL_CHANGE,
		& conmon_aud_print_msg_idset,
		"TX_CHANNEL_LABEL_CHANGE",
		NULL
	},
...
const char *
conmon_aud_print_msg__msg_type_string(
	conmon_audinate_message_type_t aud_type
)
{
	const conmon_aud_print_msg_t * info = info_for_type(aud_type);
	if (info)
	{
...
const char * CLOCK_CAPABILITY_NAMES[CONMON_AUDINATE_NUM_CLOCK_CAPABILITIES] = 
{
	...
	"RESERVED",
	"SUBDOMAIN_NAMES",
	"UNICAST_DELAY_REQUESTS(deprecated)",
	"SRATE PULLUPS HAVE SUBDOMAIN",
	"SLAVE ONLY",
	"PTP VERSION 1",
	"PTP VERSION 2",
	"DISALLOW PREFERRED",
	"PER_PORT_UNICAST_DELAY_REQUESTS"
};
...
const char * PORT_PATH_TYPE_NAMES[CONMON_AUDINATE_NUM_PORT_PATH_TYPES] =
{
	"UNKNOWN",
	"MULTICAST",
	"UNICAST",
...
const char * PORT_TRANSPORT_LAYER_NAMES[CONMON_AUDINATE_NUM_TRANSPORT_LAYERS] =
{
	"UNKNOWN",
	"LAYER2",
	"LAYER3",
...
const char * PORT_LINK_DOWN_NAMES[CONMON_AUDINATE_PORT_NUM_LINK_DOWN] =
{
	"UNKNWON",
	"LINK DOWN"
};

const char * PORT_USER_DISBALED_NAMES[CONMON_AUDINATE_PORT_NUM_USER_DISABLED] =
{
	"UNKNWON",
	"USER DISABLED"
};

const char * PORT_UDELAY_REQUEST_NAMES[CONMON_AUDINATE_PORT_NUM_UDELAY_REQUESTS] =
{
	"UNKNOWN",
	"DISABLED",
	"ENABLED"
...
#define CONMON_AUDINATE_PORT_NUM_NETWORK_INDEXES 2

const char * PORT_NETWORK_INDEX_NAMES[CONMON_AUDINATE_PORT_NUM_NETWORK_INDEXES] =
{
	"PRIMARY",
	"SECONDARY"
};
...
(
	...
	//VIA-1601: clocking status message version number is not enough to distinguish between clocking status message with ext2.
	//So we use conmon message's 'boby_size' to figure out. Maximum body size of clocking status message with conmon version < 0x700+28 is
	//sizeof(conmon_audinate_clocking_status_t)+ 2*sizeof(conmon_audinate_port_status_t) + sizeof(conmon_audinate_clocking_status_ext_t) = 88
	//A clocking status message with ext2 will have minimum body size of 96 (assuming zero ports).
	if (body_size < 90)
	{
		printf(">> num ports=%u\n", np);
		for (p = 0; p < np; p++)
	...
	else
	{
		...
		aud_error_t result = conmon_audinate_clocking_status_get_v2_domain_number(aud_msg, &domain_number);
		if (result == AUD_SUCCESS)
		{
			printf(">> v2 Domain number=%u\n", domain_number);
		}
		else
		{
			printf(">> v2 Domain number=NOT AVAILABLE\n");
		}

		uint16_t total_ports = conmon_audinate_clocking_status_total_num_ports(aud_msg);
		printf(">> num ports=%u\n", total_ports);
		for (p = 0; p < total_ports; p++)
		{
			const conmon_audinate_port_status_ext2_t * ps =	conmon_audinate_port_status_ext2_get_port_at_index(aud_msg, p);
			const conmon_audinate_port_status_ext3_t * ps_ext3 = conmon_audinate_port_status_ext3_get_port_at_index(aud_msg, p);
			uint8_t ptp_version = conmon_audinate_clocking_status_get_port_ptp_version(aud_msg, ps);
			conmon_audinate_port_path_type_t path_type = conmon_audinate_clocking_status_get_port_path_type(aud_msg, ps);
			uint16_t port_id = conmon_audinate_clocking_status_get_port_id(aud_msg, ps);
			conmon_audinate_port_transport_layer_t layer = conmon_audinate_clocking_status_get_port_transport_layer(aud_msg, ps);
			conmon_audinate_port_state_t state = conmon_audinate_clocking_status_get_port_state(aud_msg, ps);
			dante_network_index_t network_index = conmon_audinate_clocking_status_get_port_network_index(aud_msg, ps_ext3);
	
			printf(">>  port id = %d\n", port_id ? port_id : -1);
			printf(">>   state = %s\n", conmon_audinate_port_state_string(state));
			printf(">>   version = %d\n",ptp_version);
			printf(">>   path type = %s\n", (path_type < CONMON_AUDINATE_NUM_PORT_PATH_TYPES) ? PORT_PATH_TYPE_NAMES[path_type] : "UNKNOWN");
			printf(">>   transport = %s\n", (layer < CONMON_AUDINATE_NUM_TRANSPORT_LAYERS) ? PORT_TRANSPORT_LAYER_NAMES[layer]: "UNKNOWN");
			printf(">>   interface index = %d\n", conmon_audinate_clocking_status_get_port_interface_index(aud_msg, ps));
			printf(">>   network index = %s\n", (network_index < CONMON_AUDINATE_PORT_NUM_NETWORK_INDEXES) ? PORT_NETWORK_INDEX_NAMES[network_index] : "UNKNOWN");

			if (conmon_audinate_clocking_status_get_port_flag_link_down(aud_msg, ps) == CONMON_AUDINATE_PORT_LINK_DOWN)
			{
				printf(">>    link = DOWN\n");
			}
			if (conmon_audinate_clocking_status_get_port_flag_user_disabled(aud_msg, ps) == CONMON_AUDINATE_PORT_USER_DISABLED)
			{
				printf(">>    status = USER DISABLED\n");
			}
			if (path_type == CONMON_AUDINATE_PORT_PATH_TYPE_MULTICAST)
			{
				conmon_audinate_port_udelay_request_t udelay_req = conmon_audinate_clocking_status_get_port_flag_unicast_delay_request(aud_msg, ps);
				printf(">>    unicast_delay_requests = %s\n", (udelay_req < CONMON_AUDINATE_PORT_NUM_UDELAY_REQUESTS) ? PORT_UDELAY_REQUEST_NAMES[udelay_req] : "UNKNOWN");
			}
		}
	}
	fflush(stdout);
	...
	for (d = 0; d < nd; d++)
	{
		...
		if (conmon_audinate_name_id_instance_id_at_index(aud_msg, d, &instance_id))
		{
			...
			printf(">>     instance_id=%s\n", 
				conmon_instance_id_to_string(&instance_id, id_buf, sizeof(id_buf)));
		}
		if (uuid)
		{
...
)
{
	...
	unsigned int count = conmon_audinate_id_value32_num_elements(aud_msg);
	unsigned i;
	dante_dbu_t last_dbu = DANTE_DBU_UNSET;
	...
	for (i = 0; i < count; i++)
	{
		conmon_audinate_id_value32_t elem;
		dante_dbu_t dbu;

		result = conmon_audinate_id_value32_element_at_index(aud_msg,(uint16_t) i, &elem);
		if (result != AUD_SUCCESS)
		{
			fputs("<bad entry>", stdout);
			continue;
		}

		dbu = conmon_audinate_id_value32_to_int16(&elem);
		if (i)
		{
			if (dbu == last_dbu)
			{
		...
		printf("%u-%u", elem._from, elem._to);
		last_dbu = dbu;
	}

	if (last_dbu == DANTE_DBU_UNSET)
	{
		fputs(":nil\n", stdout);
	}
	else
...
const char * CAPABILITY_NAMES[CONMON_AUDINATE_NUM_CAPABILITIES] =
{
	...
	"CAN_XMODEM_UPGRADE",
	"WILL_NOT_REBOOT_AFTER_UPGRADE",
	"SUPPORTS_AES67",
	"CAN_LOCK",
	"HAS_NO_REMOTE_LOCK_RESET",
	"HAS_DDP",
	"HAS_PB",
	"HAS_USER"
};

const char * CAPABILITY1_NAMES[CONMON_AUDINATE_NUM_CAPABILITIES1] =
{
	"CAN_GET_DATA",
	"HAS_NO_PTP",
	"CAN_SEND_UPGRADE_STATUS_ADDITIONAL_TARGET",
	"HAS_CODEC_CONTROL",
	"CAN_ACCEPT_FQDN_DDM_ENROL_CONFIG",
	"HAS_MANF_OVERRIDE",
};

const char * CAPABILITY_DOMAIN_NAMES[CONMON_AUDINATE_NUM_DOMAIN_CAPABILITIES] =
{
	"SUPPORTED",
	"CAN_RESET",
	"RTP_AUDIO_SUPPORTED",
...
)
{
	...
	for(i = 0; mask; i++)
	{
		...
		if (mask & bitmask)
		{
			const char * cap_name = ((i < max_name) ? names[i] : "UNKNOWN");
			if (! (value & bitmask))
			{
				printf(">>  [0x%08x=NOT %s]\n"
					, bitmask, cap_name
...
(
	...
	uint32_t cap1 = conmon_audinate_versions_status_get_capability1_flags(aud_msg);
	conmon_audinate_message_flagset_t domain_capability_flags
		= conmon_audinate_versions_status_get_domain_capability_flags(aud_msg);
	...
	print_flags(0, cap, "capabilities", CAPABILITY_NAMES, CONMON_AUDINATE_NUM_CAPABILITIES);
	print_flags(0, cap1, "capabilities1", CAPABILITY1_NAMES, CONMON_AUDINATE_NUM_CAPABILITIES1);
	print_flags(domain_capability_flags.mask, domain_capability_flags.value,
		"domain capabilities", CAPABILITY_DOMAIN_NAMES, CONMON_AUDINATE_NUM_DOMAIN_CAPABILITIES);
	print_flags(0, icap, "inferred capabilities", CAPABILITY_NAMES, CONMON_AUDINATE_NUM_CAPABILITIES);
...
)
{
	...
	else if (file_info->protocol == CONMON_AUDINATE_UPGRADE_PROTOCOL_XMODEM)
	{
		printf(">>>> xmodem\n");
	}
	else
	{
		...
		if ((addr.in_addr != 0) && file_info->filename)
		{
			printf(
				"://%u.%u.%u.%u:%u/%s"
...
{
	dante_id64_str_t id_buf;

	fputs(dante_id64_to_hex_string(src_id, id_buf), stdout);
	id_buf[DANTE_ID64_LEN] = 0;
	fputs(" '", stdout);
	fputs(dante_id64_to_ascii_string(src_id, (char *) id_buf, sizeof(id_buf)), stdout);
	puts("'");
}
...
)
{
	...
	actions = conmon_audinate_upgrade_control_message_get_actions(aud_msg);
	printf(">> actions: 0x%x\n", actions);
	if (actions & CONMON_AUDINATE_UPGRADE_CONTROL__ACTION_DOWNLOAD)
		printf(">>> action: CONMON_AUDINATE_UPGRADE_CONTROL__ACTION_DOWNLOAD\n");
	if (actions & CONMON_AUDINATE_UPGRADE_CONTROL__ACTION_FLASH)
		printf(">>> action: CONMON_AUDINATE_UPGRADE_CONTROL__ACTION_FLASH\n");

	conmon_audinate_upgrade_source_file_t file;
	result = conmon_audinate_upgrade_control_get_source_file_info(aud_msg, body_size, &file);
	if (result == AUD_SUCCESS)
		printf(">> file_info: proto=%i, port=%i, addr=%x, file_len=%i, file='%s'\n",
				file.protocol,
				file.port,
				file.addr_inet.s_addr,
				file.file_len,
				file.filename);
	else
		printf(">> file_info: error=%i\n", result);

	dante_id64_t manf = {{0}}, model = {{0}};
	result = conmon_audinate_upgrade_control_get_override(aud_msg, &manf, &model);
	if (result == AUD_SUCCESS)
	{
		dante_id64_str_t h;
		char a[DANTE_ID64_LEN + 1];
		dante_id64_to_hex_string(&manf, h);
		dante_id64_to_ascii_string(&manf, a, sizeof(a));

		printf(">> override: manf=%s '%s', model=", h, a);

		dante_id64_to_hex_string(&model, h);
		dante_id64_to_ascii_string(&model, a, sizeof(a));
...
(
	...
	uint16_t mode;
	aud_error_t result = conmon_audinate_sys_reset_control_get_mode(aud_msg, &mode);
	if (result == AUD_SUCCESS)
	{
...
(
	...
	aud_bool_t is_enabled = conmon_audinate_aes67_status_is_enabled(aud_msg);
	aud_bool_t is_enabled_on_reboot = conmon_audinate_aes67_status_is_enabled_on_reboot(aud_msg);
	printf(">> Current: %s\n",
		(is_enabled ? "Enabled" : "Disabled")
		);
	printf(">> On Reboot: %s\n",
		(is_enabled_on_reboot ? "Enabled" : "Disabled")
		);
...
(
	...
	aud_bool_t locked = conmon_audinate_get_lock_status(aud_msg);
	printf(">> Device is: %s\n",
...
)
{
	...
	if (networks && networks->num_networks)
	{
		...
		for (i = 0; i < networks->num_networks; i++)
		{
			...
			aud_inet_ntoa(a.s_addr, in_buf, sizeof(in_buf));

			a.s_addr = n->netmask;
			aud_inet_ntoa(a.s_addr, mask_buf, sizeof(mask_buf));

			a.s_addr = n->gateway;
			aud_inet_ntoa(a.s_addr, gw_buf, sizeof(gw_buf));

			a.s_addr = n->dns_server;
			aud_inet_ntoa(a.s_addr, dns_buf, sizeof(dns_buf));
